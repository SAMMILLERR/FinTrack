using Dapper;
using FinTrack.Models;
using Microsoft.Data.SqlClient;

namespace FinTrack.Repositories;

public class BudgetRepository : IBudgetRepository
{
    private readonly IConfiguration _configuration;

    public BudgetRepository(IConfiguration configuration)
    {
        _configuration = configuration;
    }

    private SqlConnection GetConnection()
    {
        return new SqlConnection(
            _configuration.GetConnectionString("DefaultConnection"));
    }

    public async Task<IEnumerable<Budget>> GetByUserAsync(int userId)
    {
        using var connection = GetConnection();

        string sql = @"
SELECT
    b.BudgetId,
    b.UserId,
    b.CategoryId,
    c.CategoryName,
    b.BudgetMonth,
    b.BudgetYear,
    b.LimitAmount,
    ISNULL(SUM(t.Amount), 0) AS SpentAmount,
    b.CreatedOn
FROM Budgets b
INNER JOIN Categories c
    ON b.CategoryId = c.CategoryId
LEFT JOIN Transactions t
    ON b.UserId = t.UserId
    AND b.CategoryId = t.CategoryId
    AND MONTH(t.TransactionDate) = b.BudgetMonth
    AND YEAR(t.TransactionDate) = b.BudgetYear
WHERE b.UserId = @userId
GROUP BY
    b.BudgetId,
    b.UserId,
    b.CategoryId,
    c.CategoryName,
    b.BudgetMonth,
    b.BudgetYear,
    b.LimitAmount,
    b.CreatedOn
ORDER BY
    b.BudgetYear DESC,
    b.BudgetMonth DESC,
    c.CategoryName;";

        return await connection.QueryAsync<Budget>(
            sql,
            new { userId });
    }

    public async Task<Budget?> GetByIdAsync(int budgetId, int userId)
    {
        using var connection = GetConnection();

        string sql = @"
SELECT *
FROM Budgets
WHERE BudgetId = @budgetId
AND UserId = @userId;";

        return await connection.QueryFirstOrDefaultAsync<Budget>(
            sql,
            new
            {
                budgetId,
                userId
            });
    }

    public async Task AddAsync(Budget budget)
    {
        using var connection = GetConnection();

        string sql = @"
INSERT INTO Budgets
(
    UserId,
    CategoryId,
    BudgetMonth,
    BudgetYear,
    LimitAmount
)
VALUES
(
    @UserId,
    @CategoryId,
    @BudgetMonth,
    @BudgetYear,
    @LimitAmount
);";

        await connection.ExecuteAsync(sql, budget);
    }

    public async Task UpdateAsync(Budget budget)
    {
        using var connection = GetConnection();

        string sql = @"
UPDATE Budgets
SET
    CategoryId = @CategoryId,
    BudgetMonth = @BudgetMonth,
    BudgetYear = @BudgetYear,
    LimitAmount = @LimitAmount
WHERE
    BudgetId = @BudgetId
AND UserId = @UserId;";

        await connection.ExecuteAsync(sql, budget);
    }

    public async Task DeleteAsync(int budgetId, int userId)
    {
        using var connection = GetConnection();

        string sql = @"
DELETE FROM Budgets
WHERE
    BudgetId = @budgetId
AND UserId = @userId;";

        await connection.ExecuteAsync(
            sql,
            new
            {
                budgetId,
                userId
            });
    }
}